...
public class Lab1 {
	...
	public static void derivative(){
		...
		int temp=0;
 		for (int i=0;i<S.elementAt(S.size()-1).size();i++){	//进行求导（降次数）并记录常数项
			...
			if (nowstr==null) {tempn[temp++]=i;continue;}
			 if (nowstr.contains(val)){
				...
				nowstrr=(Vector<String>) nowstr.clone();	//对象类型不匹配，这个强制转换不安全
 		...
 		for (int i=0;i<temp;i++){
 			S.elementAt(S.size()-1).removeElementAt(tempn[temp]);	//删除常数项
 		}
		int printorder=0;			//输出项的次序
		for (int i=0;i<S.elementAt(S.size()-1).size();i++){		//	字符数字分开输出
			STR NOW=S.elementAt(S.size()-1).elementAt(i);	//变量名以小写字母开始比较规范
			Vector<String> nowstr=NOW.getmString();
			float nownum = NOW.getmNumber();{
				if (nownum != 0){
					if (printorder!=0 && nownum>0) {
	...
	public static void pretreatment(){	//变量名以小写字母开始比较规范
		...
		for (int i=0;i<f.length();i++){	//后缀表达式 的构建
			...
			}									//end of if ( (f.charAt(i)>='0' && f.charAt(i)<='9' )
		}										//end of for 
		...
		}								//end of if ((f.charAt(f.length()-1)>='0 
		while (ii>0){
			STA[sum].setChr(r2.charAt(ii-1));
			STA[sum].setFlag(1);
			sum++;
...
class STRINGG
{
	...
	public STRINGG(int flag, float flo, int intn, char chr, String val) {//构造函数这么写更规范点
		super();
		this.flag = flag;
		this.flo = flo;
		this.intn = intn;
